...
namespace kernels {
  ...
  void fullyConnectedBackwardCuda(const matrix_t& prevOut, const matrix_t& weights,
    ...
    engine::cuda::multiply(1.0f, false, currDelta.rows(), currDelta.cols(), currDelta.data(),
      false, weights.rows(), weights.cols(), weights.data(), 1.0f, prevDelta.data());

    engine::cuda::multiply(1.0f, true, currDelta.rows(), currDelta.cols(), currDelta.data(),
      false, prevOut.rows(), prevOut.cols(), prevOut.data(), 1.0f, dWeights.data());

    if (!dBias.isEmpty()) {
      matrix_t identity(currDelta.rows(), 1, float{1});
      engine::cuda::multiply(1.0, true, currDelta.rows(), currDelta.cols(), currDelta.data(),
