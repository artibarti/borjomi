...
namespace kernels {
  ...
  void fullyConnectedForwardThreads(const matrix_t& inData, const matrix_t& weights,
    const matrix_t& bias, matrix_t& outData) {

    if (!bias.isEmpty()) {
      engine::threads::copy(0.0, false, bias.rows(), bias.cols(),
        bias.data(), outData.rows(), outData.cols(), outData.data());
    }
    engine::threads::multiply(1.0, false, inData.rows(), inData.cols(), inData.data(),
