...
class MinPoolingLayer : public PoolingLayer {

 public:
  MinPoolingLayer(size_t inWidth, size_t inHeight, size_t inChannels,
    size_t poolingSize, engine_t engine = engine_t::internal)
    : PoolingLayer(inWidth, inHeight, inChannels, poolingSize, engine) {}

  MinPoolingLayer(MinPoolingLayer&& other) : PoolingLayer(std::move(other)) {}
  ...
  void forwardPropagation() override {
    ...
    minPoolForwardOp(getEngine(), inData, outData, getInputShape(),
  ...
  void backPropagation() override {
    ...
    minPoolBackwardOp(getEngine(), previousDelta, currentDelta,
